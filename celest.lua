...
function Leaf:CreateWindow(config)
    ...
    if type(config.Name) == "table" then
        ...
        versionName = config.Name[2]
        versionNumber = config.Name[3]
    else
        windowName = config.Name
    end
    window.versionName = versionName
    ...
    local function setActiveTab(tab)
        if activeTab then
            ...
            if activeTab.defaultSubTab then activeTab.defaultSubTab.Visible = false end
        ...
        if #activeTab.subTabs > 0 then
            ...
            if window.versionName and window.versionNumber then
                if not activeTab.defaultSubTab then
                    local SubTab = Instance.new("Frame")
                    local NameVersion = Instance.new("TextLabel")
                    local NumberVersion = Instance.new("TextLabel")
                    local subTabStroke = Instance.new("UIStroke")

                    SubTab.Name = "DefaultSubTab"
                    SubTab.Parent = Mainframe
                    SubTab.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
                    SubTab.BorderSizePixel = 0
                    SubTab.Position = UDim2.new(-0.001, 0, 1.03999996, 0)
                    ...
                    NameVersion.Name = "Name Version"
                    NameVersion.Parent = SubTab
                    NameVersion.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    NameVersion.BackgroundTransparency = 1.000
                    NameVersion.Position = UDim2.new(-0.251612902, 0, -0.800000012, 0)
                    NameVersion.Size = UDim2.new(0, 200, 0, 50)
                    NameVersion.Font = Enum.Font.GothamBold
                    NameVersion.Text = window.versionName
                    NameVersion.TextColor3 = Color3.fromRGB(255, 255, 255)
                    NameVersion.TextSize = 14.000

                    NumberVersion.Name = "Number Version"
                    NumberVersion.Parent = SubTab
                    NumberVersion.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    NumberVersion.BackgroundTransparency = 1.000
                    NumberVersion.Position = UDim2.new(0.609677434, 0, -0.800000012, 0)
                    NumberVersion.Size = UDim2.new(0, 200, 0, 50)
                    NumberVersion.Font = Enum.Font.GothamBold
                    NumberVersion.Text = window.versionNumber
                    NumberVersion.TextColor3 = Color3.fromRGB(255, 255, 255)
                    NumberVersion.TextSize = 14.000
                    
                    activeTab.defaultSubTab = SubTab
                end
                activeTab.defaultSubTab.Visible = true
            end
        end
    ...
    function window:CreateTab(props)
        ...
        local function setActiveSubTab(stab)
            ...
            self.activeSubTab.ScrollingFrame.Visible = true
        end
