...
function Leaf:CreateWindow(config)
    ...
    local windowName = type(config.Name) == "table" and config.Name[1] or config.Name
    ...
    Leaf.MenuColorValue.Changed:Connect(function()
        for _, item in ipairs(Leaf.colorElements) do item.element[item.property] = Leaf.MenuColorValue.Value end
        for _, toggleData in ipairs(Leaf.toggles) do toggleData.update() end
    end)
    
    local MiniMenu = Instance.new("ScreenGui", game:GetService("CoreGui"))
    MiniMenu.Name = "MiniMenu"
    MiniMenu.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    
    local MiniMenuFrame = Instance.new("Frame", MiniMenu)
    ...
    local UICornerMini = Instance.new("UICorner", MiniMenuFrame)
    UICornerMini.CornerRadius = UDim.new(0, 4)
    
    local ImageMiniMenu = Instance.new("ImageLabel", MiniMenuFrame)
    ...
    local Bmenu = Instance.new("TextButton", MiniMenuFrame)
    Bmenu.Name = "Bmenu"
    ...
    local UIStroke1 = Instance.new("UIStroke", OuterFrame)
    ...
    local UIStroke2 = Instance.new("UIStroke", InnerFrame)
    UIStroke2.Color = Color3.fromRGB(80, 80, 80)
    UIStroke2.Thickness = 2
    
    local Mainframe = Instance.new("Frame", InnerFrame)
    ...
    local UIStroke3 = Instance.new("UIStroke", Mainframe)
    ...
    local UIStroke4 = Instance.new("UIStroke", TopBar)
    ...
    local function updateTabPositions()
        ...
        for i, tab in ipairs(allTabs) do tab.TabButton.Position = UDim2.new(0, startX + (i - 1) * (buttonWidth + gap), 0.073, 0) end
    end
    ...
    local function setActiveTab(tab)
        if activeTab then
            ...
            for i, item in ipairs(Leaf.colorElements) do if item.element == activeTab.TabButton then table.remove(Leaf.colorElements, i) break end end
        end
    ...
    function window:CreateTab(props)
        ...
        local tab = {}
        tab.window = self
        local TabButton = Instance.new("ImageButton", TopBar)
        ...
        if Leaf.versionName and Leaf.versionNumber then
            local versionBar = Instance.new("Frame", Mainframe)
            ...
            local numberVersionLabel = Instance.new("TextLabel", versionBar)
        ...
        function tab:CreateSubTab(subProps)
            ...
            if not self.subTabs then
                ...
                self.subTabFrame = Instance.new("Frame", Mainframe)
                ...
                local subTabStroke = Instance.new("UIStroke", self.subTabFrame)
            ...
            local subTabButton = Instance.new("TextButton", self.subTabFrame)
            ...
            local subTabScrollingFrame = Instance.new("ScrollingFrame", Mainframe)
            ...
            local function updateSubTabPositions()
                ...
                local totalWidth = 0
                for _, sub in ipairs(self.subTabs) do totalWidth = totalWidth + sub.button.AbsoluteSize.X end
                local spacing = (self.subTabFrame.AbsoluteSize.X - totalWidth) / (numSubTabs + 1)
                local currentX = spacing
                for _, sub in ipairs(self.subTabs) do sub.button.Position = UDim2.new(0, currentX, 0, 0) currentX = currentX + sub.button.AbsoluteSize.X + spacing end
            end
            ...
            local function setActiveSubTab(stab)
                if self.activeSubTab then
                    ...
                    for i, item in ipairs(Leaf.colorElements) do if item.element == self.activeSubTab.button then table.remove(Leaf.colorElements, i) break end end
                else self.ScrollingFrame.Visible = false end
            ...
            subTabButton.MouseButton1Click:Connect(function() setActiveSubTab(subTab) end)
            if subProps.Opened then setActiveSubTab(subTab) end
        ...
        function tab:Button(props)
            local ButtonFrame = Instance.new("Frame", self.ScrollingFrame)
            ButtonFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
            ButtonFrame.Size = UDim2.new(0, 280, 0, 40)
            ButtonFrame.Position = UDim2.new(0.5, -140, 0, self.nextPosition)
            local UICornerBtn = Instance.new("UICorner", ButtonFrame)
            UICornerBtn.CornerRadius = UDim.new(0, 4)
            
            local Indicator = Instance.new("Frame", ButtonFrame)
            ...
            local UICornerInd = Instance.new("UICorner", Indicator)
            UICornerInd.CornerRadius = UDim.new(0, 4)
            
            local NameButton = Instance.new("TextLabel", ButtonFrame)
            ...
            local TextButton = Instance.new("TextButton", ButtonFrame)
            ...
            TextButton.MouseButton1Click:Connect(function()
                ...
                while os.clock() - startTime < (props.Active or 0.5) do runService.Heartbeat:Wait() end
                if clickCount == currentClick then Indicator.BackgroundColor3 = Color3.fromRGB(30, 30, 30) end
            end)
        ...
        function tab:DeButton(props)
            local DeButtonFrame = Instance.new("Frame", self.ScrollingFrame)
            ...
            local UICornerDeBtn = Instance.new("UICorner", DeButtonFrame)
            UICornerDeBtn.CornerRadius = UDim.new(0, 4)
            
            local NameButton = Instance.new("TextLabel", DeButtonFrame)
            NameButton.BackgroundTransparency = 1
            ...
            local TextButton = Instance.new("TextButton", DeButtonFrame)
            TextButton.BackgroundTransparency = 1
            TextButton.Size = UDim2.new(1, 0, 1, 0)
            TextButton.Text = ""
            TextButton.MouseButton1Click:Connect(function() if props.Callback then pcall(props.Callback) end end)
            self.nextPosition = self.nextPosition + 45
            self.ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, self.nextPosition + 10)
        end

        function tab:Toggle(props)
            local ToggleFrame = Instance.new("Frame", self.ScrollingFrame)
            ToggleFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
            ToggleFrame.Size = UDim2.new(0, 280, 0, 40)
            ToggleFrame.Position = UDim2.new(0.5, -140, 0, self.nextPosition)
            local UICornerTog = Instance.new("UICorner", ToggleFrame)
            ...
            local UICornerCir = Instance.new("UICorner", Circle)
            UICornerCir.CornerRadius = UDim.new(1, 0)
            
            local NameButton = Instance.new("TextLabel", ToggleFrame)
            NameButton.BackgroundTransparency = 1
            NameButton.Position = UDim2.new(0.04, 0, 0, 0)
            ...
            local TextButton = Instance.new("TextButton", ToggleFrame)
            TextButton.BackgroundTransparency = 1
            TextButton.Size = UDim2.new(1, 0, 1, 0)
            TextButton.Text = ""
        ...
        function tab:Slider(props)
            ...
            local SliderFrame = Instance.new("Frame", self.ScrollingFrame)
            SliderFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
            SliderFrame.Size = UDim2.new(0, 280, 0, 45)
            SliderFrame.Position = UDim2.new(0.5, -140, 0, self.nextPosition)
            local UICornerSld = Instance.new("UICorner", SliderFrame)
            ...
            local UICornerFill = Instance.new("UICorner", Fill)
            UICornerFill.CornerRadius = UDim.new(0, 4)
            
            local Progress = Instance.new("Frame", Fill)
            ...
            local UICornerProg = Instance.new("UICorner", Progress)
            UICornerProg.CornerRadius = UDim.new(0, 4)
            
            local Snumber = Instance.new("TextLabel", SliderFrame)
        ...
        function tab:Section(props)
            local SectionFrame = Instance.new("Frame", self.ScrollingFrame)
            ...
            local UICornerSec = Instance.new("UICorner", SectionFrame)
            ...
            local Underline = Instance.new("Frame", SectionFrame)
        ...
        function tab:CreateDropdown(props)
            ...
            local DropdownFrame = Instance.new("Frame", self.ScrollingFrame)
            ...
            local TextButton = Instance.new("TextButton", DropdownFrame)
            TextButton.BackgroundTransparency = 1
            TextButton.Size = UDim2.new(1, 0, 1, 0)
            TextButton.Text = ""
            
            local Info = Instance.new("TextButton", DropdownFrame)
            ...
            Info.TextSize = 14
            local UICornerInfo = Instance.new("UICorner", Info)
            UICornerInfo.CornerRadius = UDim.new(0, 4)
            
            local DropdownList = Instance.new("Frame", OuterFrame)
            ...
            local ScrollingFrameList = Instance.new("ScrollingFrame", DropdownList)
            ...
            local UIListLayout = Instance.new("UIListLayout", ScrollingFrameList)
            UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
            UIListLayout.Padding = UDim.new(0, 5)
            
            local function createOption(option)
                local OptionFrame = Instance.new("Frame", ScrollingFrameList)
            ...
            for _, option in ipairs(props.Options) do createOption(option) end
            ...
            function dropdownObject:UpdateOptions(newOptions)
                for _, child in ipairs(ScrollingFrameList:GetChildren()) do if child:IsA("Frame") then child:Destroy() end end
                props.Options = newOptions
                for _, option in ipairs(newOptions) do createOption(option) end
                ScrollingFrameList.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y)
                local currentOption = Info.Text
                local found = false
                for _, option in ipairs(newOptions) do if option == currentOption then found = true break end end
                if not found and #newOptions > 0 then Info.Text = newOptions[1] end
            end
            ...
            self.window.elements[key] = {
                ...
                SetValue = function(value)
                    local found = false
                    for _, option in ipairs(props.Options) do if option == value then found = true break end end
                    if found then
                        Info.Text = value
        ...
        function tab:CreateColorPicker(props)
            ...
            local ColorPickerFrame = Instance.new("Frame", self.ScrollingFrame)
            ...
            local UICornerCP = Instance.new("UICorner", ColorPickerFrame)
            UICornerCP.CornerRadius = UDim.new(0, 4)
            
            local NameLabel = Instance.new("TextLabel", ColorPickerFrame)
            ...
            local ColorIndicator = Instance.new("Frame", ColorPickerFrame)
            ...
            local UICornerCI = Instance.new("UICorner", ColorIndicator)
            UICornerCI.CornerRadius = UDim.new(0, 4)
            
            local PickButton = Instance.new("TextButton", ColorPickerFrame)
            PickButton.BackgroundTransparency = 1
            PickButton.Size = UDim2.new(1, 0, 1, 0)
            PickButton.Text = ""
            
            local ChangeColor = Instance.new("Frame", ScreenGui)
            ...
            local TopBarCP = Instance.new("Frame", ChangeColor)
            ...
            local UICornerTopBarCP = Instance.new("UICorner", TopBarCP)
            UICornerTopBarCP.CornerRadius = UDim.new(0,4)
            
            local TopBarTitle = Instance.new("TextLabel", TopBarCP)
            ...
            local HueSlider = Instance.new("Frame", ChangeColor)
            ...
            local UICornerHue = Instance.new("UICorner", HueSelector)
            UICornerHue.CornerRadius = UDim.new(1, 0)
            local UIStrokeHue = Instance.new("UIStroke", HueSelector)
            ...
            local UICornerSel = Instance.new("UICorner", ColorSelector)
            UICornerSel.CornerRadius = UDim.new(1, 0)
            local UIStrokeSel = Instance.new("UIStroke", ColorSelector)
            UIStrokeSel.Thickness = 2
            UIStrokeSel.Color = Color3.new(1, 1, 1)
            
            local ApplyButton = Instance.new("TextButton", ChangeColor)
            ...
            local UICornerApply = Instance.new("UICorner", ApplyButton)
            UICornerApply.CornerRadius = UDim.new(0, 4)
            
            local CancelButton = Instance.new("TextButton", ChangeColor)
            ...
            local UICornerCancel = Instance.new("UICorner", CancelButton)
            UICornerCancel.CornerRadius = UDim.new(0, 4)
            
            local MainGradient = Instance.new("UIGradient")
            MainGradient.Rotation = 0
            MainGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)), ColorSequenceKeypoint.new(1, Color3.fromHSV(0, 1, 1))}
            ...
            local draggingHue, draggingColor, draggingCP = false, false, false
            ...
            local function updateHueSelector(input)
                ...
                MainGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)), ColorSequenceKeypoint.new(1, Color3.fromHSV(currentHue, 1, 1))}
                updateColor()
            end
            ...
            HueSlider.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingHue = true updateHueSelector(input) end end)
            ColorCanvas.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingColor = true updateColorSelector(input) end end)
            TopBarCP.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingCP = true dragStartCP = input.Position startPosCP = ChangeColor.Position end end)
            
            game:GetService("UserInputService").InputChanged:Connect(function(input)
                if draggingHue and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then updateHueSelector(input)
                elseif draggingColor and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then updateColorSelector(input)
                elseif draggingCP and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                    ...
                    ChangeColor.Position = UDim2.new(startPosCP.X.Scale, startPosCP.X.Offset + delta.X, startPosCP.Y.Scale, startPosCP.Y.Offset + delta.Y)
                end
            end)
            
            game:GetService("UserInputService").InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingHue, draggingColor, draggingCP = false, false, false end
            end)
            ...
            self.window.elements[key] = {
                GetValue = function() local c = ColorIndicator.BackgroundColor3 return {c.R, c.G, c.B} end,
                SetValue = function(value)
                    local color
                    if type(value) == "table" and #value == 3 then color = Color3.new(value[1], value[2], value[3])
                    elseif typeof(value) == "Color3" then color = value else return end
                    Color = color
                    ColorIndicator.BackgroundColor3 = color
        ...
        function tab:Input(props)
            local key = props.Key or props.Title
            local InputFrame = Instance.new("Frame", self.ScrollingFrame)
            ...
            local UICornerInp = Instance.new("UICorner", InputFrame)
            ...
            local UICornerInputBox = Instance.new("UICorner", InputBox)
            UICornerInputBox.CornerRadius = UDim.new(0, 4)
            
            InputBox.FocusLost:Connect(function(enterPressed) if props.Callback then pcall(props.Callback, InputBox.Text) end end)
            self.window.elements[key] = {
                GetValue = function() return InputBox.Text end,
                SetValue = function(value) InputBox.Text = value if props.Callback then pcall(props.Callback, value) end end
            }
            self.nextPosition = self.nextPosition + 45
            self.ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, self.nextPosition + 10)
        end
        
        if props.Opened then setActiveTab(tab) else ScrollingFrame.Visible = false end
    ...
    Bmenu.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then miniMenuDragging = false end
    end)
    UserInputService.InputChanged:Connect(function(input)
    ...
    Bmenu.MouseButton1Click:Connect(function() ScreenGui.Enabled = not ScreenGui.Enabled end)
    return window
end
