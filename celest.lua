...
function Leaf:CreateWindow(config)
    ...
    function window:CreateTab(props)
        ...
        function tab:CreateSubTab(subProps)
            ...
            subTab.Opened = subProps.Opened or false
            subTab.SubTabFrame = Instance.new("Frame")
            subTab.SubTabFrame.Name = "SubTabFrame"
            subTab.SubTabFrame.Parent = self.ScrollingFrame
            subTab.SubTabFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
            subTab.SubTabFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
            subTab.SubTabFrame.BorderSizePixel = 0
            subTab.SubTabFrame.Position = UDim2.new(0, 0, 0, self.nextPosition)
            subTab.SubTabFrame.Size = UDim2.new(0, 310, 0, 20)
            local SubTabStroke = Instance.new("UIStroke")
            SubTabStroke.Parent = subTab.SubTabFrame
            SubTabStroke.Color = Color3.fromRGB(60, 60, 60)
            SubTabStroke.Thickness = 2
            SubTabStroke.LineJoinMode = Enum.LineJoinMode.Round
            subTab.Buttons = {}
            subTab.SelectedButtonIndex = nil
            local function updateSubTabLayout()
                local numButtons = #subTab.Buttons
                if numButtons == 0 then return end
                local buttonWidth = 78
                local totalWidth = buttonWidth * numButtons
                local spacing = (310 - totalWidth) / (numButtons + 1)
                spacing = math.max(spacing, 5)
                local totalCalcWidth = buttonWidth * numButtons + spacing * (numButtons - 1)
                local startX = (310 - totalCalcWidth) / 2
                for i, btn in ipairs(subTab.Buttons) do
                    btn.Position = UDim2.new(0, startX + (i - 1) * (buttonWidth + spacing), 0, 0)
                    btn.Size = UDim2.new(0, buttonWidth, 1, 0)
                    if i == subTab.SelectedButtonIndex then
                        btn.TextSize = 14
                    else
                        btn.TextSize = 13
            ...
            function subTab:Select(index)
                if subTab.Buttons[index] then
                    if subTab.SelectedButtonIndex then
                        subTab.Buttons[subTab.SelectedButtonIndex].TextSize = 13
                        subTab.Buttons[subTab.SelectedButtonIndex].TextColor3 = Color3.fromRGB(130, 130, 130)
                    end
                    subTab.SelectedButtonIndex = index
                    local selectedBtn = subTab.Buttons[index]
            ...
            function subTab:AddButton(name, callback)
                ...
                btn.TextColor3 = Color3.fromRGB(130, 130, 130)
                btn.TextSize = 13.000
                local index = #subTab.Buttons + 1
                subTab.Buttons[index] = btn
                if subTab.Opened and subTab.SelectedButtonIndex == nil then
                    subTab:Select(index)
                end
            ...
            self.nextPosition = self.nextPosition + 25
