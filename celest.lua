...
function Leaf:CreateWindow(config)
    local window = {}
    window.elements = {}
    window.subTabs = {} -- Хранилище для всех SubTab
    ...
    Mainframe.Size = UDim2.new(0, 310, 0, 200) -- Высота уменьшена для размещения SubTab
    ...
    local function setActiveTab(tab)
        if activeTab then
            ...
            -- Скрываем SubTab при переключении основных вкладок
        ...
        activeTab.TabButton.ImageColor3 = Leaf.MenuColorValue.Value
        -- Показываем SubTab текущей вкладки, если он есть
        if activeTab.SubTabFrame then
    ...
    function window:CreateTab(props)
        ...
        local tab = {}
        tab.window = self
        tab.subTabs = {} -- Хранилище SubTab для этой вкладки
        tab.activeSubTab = nil -- Активная SubTab для этой вкладки
        ...
        ScrollingFrame.Size = UDim2.new(0, 309, 0, 175) -- Уменьшена высота для размещения SubTab
        ScrollingFrame.Visible = props.Opened
        ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
        ScrollingFrame.ScrollBarThickness = 3

        -- Создаем контейнер для SubTab
        local SubTabFrame = Instance.new("Frame")
        SubTabFrame.Name = "SubTabFrame"
        SubTabFrame.Parent = Mainframe
        SubTabFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        SubTabFrame.BorderSizePixel = 0
        SubTabFrame.Position = UDim2.new(0, 0, 0, 175) -- Располагаем под ScrollingFrame
        SubTabFrame.Size = UDim2.new(0, 310, 0, 25)
        SubTabFrame.Visible = false -- По умолчанию скрыт
        ...
        tab.SubTabFrame = SubTabFrame -- Сохраняем ссылку на SubTabFrame
        tab.nextPosition = 10

        -- Функция для установки активной SubTab
        local function setActiveSubTab(subTab)
            if tab.activeSubTab then
                ...
                -- Скрываем содержимое предыдущей SubTab
                if tab.activeSubTab.ContentFrame then
                    tab.activeSubTab.ContentFrame.Visible = false
                end
            end
            ...
            if subTab then
                ...
                subTab.Button.TextColor3 = Color3.fromRGB(255, 255, 255)
                subTab.Button.TextSize = 14
                -- Показываем содержимое активной SubTab
                if subTab.ContentFrame then
                    subTab.ContentFrame.Visible = true
                end
            end
        end

        -- Функция для обновления позиций кнопок SubTab
        local function updateSubTabPositions()
            local n = #tab.subTabs
            if n == 0 then return end
            
            local buttonWidth = 78
            local totalButtonsWidth = buttonWidth * n
            local totalGaps = 10 * (n - 1)
            local totalContentWidth = totalButtonsWidth + totalGaps
            local startX = (310 - totalContentWidth) / 2
            
            for i, subTab in ipairs(tab.subTabs) do
                local xPos = startX + (i - 1) * (buttonWidth + 10)
                subTab.Button.Position = UDim2.new(0, xPos, 0, 0)
            end
        end
        ...
        function tab:CreateSubTab(subProps)
            ...
            -- Создаем фрейм для содержимого SubTab
            local contentFrame = Instance.new("Frame")
            contentFrame.Parent = self.ScrollingFrame
            contentFrame.BackgroundTransparency = 1
            contentFrame.Size = UDim2.new(1, 0, 1, 0)
            contentFrame.Visible = false
            contentFrame.Name = subProps.Name .. "Content"
            
            subTab.Button = subTabButton
            subTab.ContentFrame = contentFrame
            subTab.nextPosition = 10 -- Начальная позиция для элементов внутри SubTab
            
            -- Обработчик клика по кнопке SubTab
            subTabButton.MouseButton1Click:Connect(function()
                setActiveSubTab(subTab)
            end)
            
            -- Добавляем SubTab в список
            table.insert(self.subTabs, subTab)
            
            -- Обновляем позиции кнопок
            updateSubTabPositions()
            
            -- Если это первая SubTab или указана как открытая по умолчанию, делаем её активной
            if #self.subTabs == 1 or subProps.Opened then
                setActiveSubTab(subTab)
                self.SubTabFrame.Visible = true
            end
            
            -- Возвращаем объект SubTab с методами для добавления элементов
            local subTabMethods = {}
            
            -- Общий метод для создания элементов внутри SubTab
            local function createElement(elementType, props, createFunction)
                if not self.activeSubTab or self.activeSubTab ~= subTab then
                    -- Если эта SubTab не активна, временно делаем её активной для корректного создания элемента
                    local wasActive = self.activeSubTab
                    setActiveSubTab(subTab)
                    
                    local element = createFunction(contentFrame, subTab.nextPosition, props)
                    
                    -- Восстанавливаем предыдущую активную SubTab
                    if wasActive then
                        setActiveSubTab(wasActive)
                    end
                    
                    return element
                else
                    return createFunction(contentFrame, subTab.nextPosition, props)
                end
            end
            
            function subTabMethods:Button(props)
                local function createButton(parent, position, props)
                    ...
                    ButtonFrame.Position = UDim2.new(0.5, -140, 0, position)
                    ...
                    parent.Parent.CanvasSize = UDim2.new(0, 0, 0, subTab.nextPosition + 10)
                ...
                return createElement("Button", props, createButton)
            end
            
            function subTabMethods:Toggle(props)
                local function createToggle(parent, position, props)
                    ...
                    ToggleFrame.Position = UDim2.new(0.5, -140, 0, position)
                ...
                return createElement("Toggle", props, createToggle)
            end
            
            function subTabMethods:Slider(props)
                local min = props.Value.Min
                local max = props.Value.Max
                local increment = props.Value.Increment
                local default = props.Value.Default
                
                local function createSlider(parent, position, props)
                    ...
                    SliderFrame.Position = UDim2.new(0.5, -140, 0, position)
                    ...
                    subTab.nextPosition = subTab.nextPosition + 50 
                    parent.Parent.CanvasSize = UDim2.new(0, 0, 0, subTab.nextPosition + 10)
                ...
                return createElement("Slider", props, createSlider)
            end
            
            function subTabMethods:Section(props)
                local function createSection(parent, position, props)
                    ...
                    SectionFrame.Position = UDim2.new(0.5, -140, 0, position)
                    ...
                    subTab.nextPosition = subTab.nextPosition + 30
                    parent.Parent.CanvasSize = UDim2.new(0, 0, 0, subTab.nextPosition + 10)
                ...
                return createElement("Section", props, createSection)
            end
            
            function subTabMethods:CreateDropdown(props)
                local function createDropdown(parent, position, props)
                    ...
                    DropdownFrame.Position = UDim2.new(0.5, -140, 0, position)
                ...
                return createElement("Dropdown", props, createDropdown)
            end
            
            function subTabMethods:CreateColorPicker(props)
                local function createColorPicker(parent, position, props)
                    ...
                    ColorPickerFrame.Position = UDim2.new(0.5, -140, 0, position)
                    ...
                    return {
                        ...
                        Button = PickButton,
                        PickerFrame = ChangeColor
                    }
                end
                
                return createElement("ColorPicker", props, createColorPicker)
            end
            
            function subTabMethods:Input(props)
                local function createInput(parent, position, props)
                    ...
                    InputFrame.Position = UDim2.new(0.5, -140, 0, position)
                ...
                return createElement("Input", props, createInput)
            end
        ...
        if props.Opened then
            activeTab = tab
            SubTabFrame.Visible = true -- Показываем SubTabFrame если вкладка открыта
        else
