...
function Leaf:CreateWindow(config)
    ...
    local function setActiveSubTab(tab, subtab)
        if tab.ActiveSubTab then
        ...
        if tab == activeTab then
            subtab.scrollingFrame.Visible = true
        end
    end
    ...
    local function setActiveTab(tab)
        if activeTab then
            ...
            if activeTab.SubTabBar then
                activeTab.SubTabBar.Visible = false
        ...
        activeTab = tab
        activeTab.TabButton.ImageColor3 = Leaf.MenuColorValue.Value
        if activeTab.SubTabBar and activeTab.SubTabBar.Visible then
            activeTab.SubTabBar.Visible = true
            if activeTab.ActiveSubTab then
    ...
    function window:CreateTab(props)
        ...
        function tab:CreateSubTab(props)
            ...
            for i, st in ipairs(self.SubTabs) do
                st.button.Position = UDim2.new(0, startX + (i-1)*(buttonWidth+gap), 0, 0)
            end
            button.MouseButton1Click:Connect(function()
                setActiveSubTab(self, subtab)
            end)
